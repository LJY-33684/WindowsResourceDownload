    ...
    override suspend fun propFind(
        ...
        response.use { currentResponse ->
            ...
            try {
                ...
                println(
                    "Unable to parse WebDAV PROPFIND response; Content-Type=$mediaType; Content-Encoding=$contentEncoding; error=${error.message}",
                )
    ...
    override suspend fun move(
        ...
        executeWrite { lease ->
            Request.Builder()
                ...
                .header("Overwrite", if (overwrite) "T" else "F")
                .apply { if (sourceEtag != null) header("If-Match", sourceEtag) }
                .method("MOVE", null)
                .build()
